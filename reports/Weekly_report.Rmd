---
title: "Covid in Jails- Redcap Data Cleaning" 
author: "Week of 10-19-2020"
date: "`r Sys.Date()`"
output: 
  word_document:
    toc: true
...
enrollment_data_cleaned %>% 
  filter(consent_yn == 1, year(consent_date) == "2020", this_week == 1) %>% 
  ggplot(aes(fill = population, x = results_date)) +
...
enrollment_data_cleaned %>% 
  filter(consent_yn == 1, year(consent_date) == "2020") %>% 
...
- second full week of testing 
- With wifi - > no probblems with Redcap

## Challenges 

- Running low on premier tests and purple lid tubes 
